...
import { ChevronDown, PlusCircle, Edit, Scan } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === "manual" ? (
          ...
          <label className="flex items-center gap-3 px-1 cursor-pointer select-none">
            <div
              className={`w-11 h-6 rounded-full transition-colors relative ${
                isCredit ? "bg-green-500" : "bg-gray-200"
              }`}
              onClick={() => setIsCredit((v) => !v)}
            >
              <span
                className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
                  isCredit ? "translate-x-5.5 left-0.5" : "left-0.5"
                }`}
              />
            </div>
        ...
        <ScanAndPay  onClose={() => setMode("manual")}  />
      )}
